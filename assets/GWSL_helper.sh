...
then
	...
	then
		...
		then
			# Configure Fish
			sed -i.bak '/DISPLAY /d' ~/.config/fish/config.fish
			echo "set -gx DISPLAY (ip route show | grep default | awk '{print $3}'):0.0 #GWSL" >> ~/.config/fish/config.fish
		elif [ $wsl_shell == "zsh" ]
		then
			# Configure Zsh
			sed -i.bak '/DISPLAY=/d' ~/.zshrc
			echo "export DISPLAY=\$(ip route show | grep default | awk '{print $3}'):0.0 #GWSL" >> ~/.zshrc
		elif [ $wsl_shell == "bash" ]
		then
			# Configure Bash
			sed -i.bak '/DISPLAY=/d' ~/.profile
			echo "export DISPLAY=\$(ip route show | grep default | awk '{print $3}'):0.0 #GWSL" >> ~/.profile
			sed -i.bak '/DISPLAY=/d' ~/.bashrc
			echo "export DISPLAY=\$(ip route show | grep default | awk '{print $3}'):0.0 #GWSL" >> ~/.bashrc
		fi
	fi
...
then
	...
	then
		...
		then
			# Configure Fish
			sed -i.bak '/PULSE_SERVER /d' ~/.config/fish/config.fish
			echo "set -gx PULSE_SERVER tcp:(ip route show | grep default | awk '{print $3}') #GWSL" >> ~/.config/fish/config.fish
		elif [ $wsl_shell == "zsh" ]
		then
			# Configure Zsh
			sed -i.bak '/PULSE_SERVER=/d' ~/.zshrc
			echo "export PULSE_SERVER=tcp:\$(ip route show | grep default | awk '{print $3}') #GWSL" >> ~/.zshrc
		elif [ $wsl_shell == "bash" ]
		then
			# Configure Bash
			sed -i.bak '/PULSE_SERVER=/d' ~/.profile
			echo "export PULSE_SERVER=tcp:\$(ip route show | grep default | awk '{print $3}') #GWSL" >> ~/.profile
			sed -i.bak '/PULSE_SERVER=/d' ~/.bashrc
			echo "export PULSE_SERVER=tcp:\$(ip route show | grep default | awk '{print $3}') #GWSL" >> ~/.bashrc
		fi
	fi
